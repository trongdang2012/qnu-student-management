...
<div class="admin-wrapper">
  <div class="admin-container">
    ...
    <div class="page-title fade-in">
      ...
      <h1><i class="fas fa-landmark"></i> Quản lý danh mục Khoa</h1>
      <p>Quản lý các khoa đào tạo trong toàn trường Đại học Quy Nhơn.</p>
    </div>
    ...
    <div class="list-card fade-in">
      ...
      <div class="card-header" style="background: #fafafa; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px; border-bottom: 1px solid var(--border);">
        <div style="display: flex; align-items: center; gap: 10px;">
          <h3 style="margin: 0; color: var(--primary);">Danh sách Khoa</h3>
          <span class="badge-count"><?= $total ?> khoa</span>
        ...
        <form method="GET" style="display: flex; gap: 10px;">
          <input type="text" name="search" class="form-control" placeholder="Tìm kiếm khoa..." value="<?= e($search) ?>" style="width: 220px; font-size: 14px;">
          <button type="submit" class="btn btn-primary btn-sm"><i class="fas fa-search"></i> Tìm</button>
          <?php if (!empty($search)): ?>
            <a href="<?= BASE_URL ?>/admin/khoa" class="btn btn-secondary btn-sm"><i class="fas fa-sync-alt"></i></a>
          <?php endif; ?>
        </form>
      </div>
            ...
            <?php else: ?>
              ...
              ?>
                <tr class="hover-row">
                  ...
                  <td style="font-weight: 500; color: var(--primary);"><?= e($f['ten_khoa']) ?></td>
                  <td style="text-align: center;">
                    <div style="display: inline-flex; gap: 8px;">
                      <button type="button" class="btn btn-outline btn-sm" onclick="editFaculty(<?= $f['id'] ?>, '<?= e($f['ten_khoa']) ?>')">
                        <i class="fas fa-edit"></i> Sửa
                      </button>
                      <form action="<?= BASE_URL ?>/admin/khoa/delete" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xóa khoa này? Tất cả ngành và lớp trực thuộc sẽ bị xóa theo!');" style="margin: 0;">
...
<!-- MODAL THÊM/SỬA KHOA -->
<div class="crud-modal-overlay" id="facultyModal">
  <div class="crud-modal">
    <div class="crud-modal-header">
      <h3 id="modal-title"><i class="fas fa-plus-circle"></i> Thêm Khoa mới</h3>
      <button class="close-btn" type="button" onclick="closeModal()">&times;</button>
    </div>
    <form action="<?= BASE_URL ?>/admin/khoa/save" method="POST" id="faculty-form">
      <input type="hidden" name="id" id="faculty-id" value="0">
      <div class="crud-modal-body">
        <div class="form-group">
          <label for="ten_khoa" style="font-weight: 500; display: block; margin-bottom: 8px;">Tên khoa <span style="color: red;">*</span></label>
          <input type="text" name="ten_khoa" id="ten_khoa" class="form-control" placeholder="VD: Kỹ thuật - Công nghệ" required style="width: 100%;">
        </div>
      </div>
...
<script>
function openModal() {
  document.getElementById('modal-title').innerHTML = '<i class="fas fa-plus-circle"></i> Thêm Khoa mới';
  document.getElementById('faculty-id').value = '0';
  document.getElementById('ten_khoa').value = '';
  document.getElementById('facultyModal').classList.add('active');
  setTimeout(() => document.getElementById('ten_khoa').focus(), 200);
}

function editFaculty(id, name) {
  document.getElementById('modal-title').innerHTML = '<i class="fas fa-edit"></i> Chỉnh sửa Khoa';
  document.getElementById('faculty-id').value = id;
  document.getElementById('ten_khoa').value = name;
  document.getElementById('facultyModal').classList.add('active');
  setTimeout(() => document.getElementById('ten_khoa').focus(), 200);
}
