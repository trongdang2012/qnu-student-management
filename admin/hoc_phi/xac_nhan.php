<?php
/**
 * admin/hoc_phi/xac_nhan.php - Xác nhận sinh viên đã nộp học phí
 */
define('ROOT', __DIR__ . '/../../');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'confirm') {
    ...
    if (empty($selected)) {
        ...
        $stmt = $db->prepare("UPDATE hoc_phi SET da_nop = so_tien, trang_thai = 'Đã nộp' WHERE id IN ($placeholders)");
        $stmt->bind_param($types, ...$selected);
        if ($stmt->execute()) {
            setFlash('success', 'Xác nhận đã nộp học phí cho ' . $stmt->affected_rows . ' bản ghi.');
        } else {
            setFlash('danger', 'Lỗi khi xác nhận: ' . $stmt->error);
        }
        $stmt->close();
    }
    header('Location: ' . BASE_URL . '/admin/hoc_phi/xac_nhan.php');
    exit;
}

$id = (int)($_GET['id'] ?? 0);
if ($id > 0 && ($_GET['action'] ?? '') === 'mark') {
    $stmt = $db->prepare("UPDATE hoc_phi SET da_nop = so_tien, trang_thai = 'Đã nộp' WHERE id = ? LIMIT 1");
    $stmt->bind_param('i', $id);
    $stmt->execute();
    setFlash('success', $stmt->affected_rows > 0 ? 'Đã xác nhận nộp học phí cho bản ghi.' : 'Không tìm thấy bản ghi để xác nhận.');
    $stmt->close();
    header('Location: ' . BASE_URL . '/admin/hoc_phi/xac_nhan.php');
    exit;
}

$pendingFees = $db->query("SELECT hf.*, sv.ma_sv, sv.ho_ten, sv.khoa, sv.nganh, sv.lop FROM hoc_phi hf JOIN sinh_vien sv ON sv.id = hf.sinh_vien_id WHERE hf.trang_thai IN ('Chưa nộp', 'Nợ') ORDER BY sv.khoa, sv.nganh, sv.lop, hf.nam_hoc DESC, hf.hoc_ky DESC")->fetch_all(MYSQLI_ASSOC);
                ...
                <?php else: ?>
                  <?php foreach ($pendingFees as $fee): ?>
                    <tr>
                      ...
                      <td style="text-align:center"><a class="btn btn-sm btn-success" href="<?= BASE_URL ?>/admin/hoc_phi/xac_nhan.php?action=mark&id=<?= (int)$fee['id'] ?>">Đã nộp</a></td>
                    </tr>
                  <?php endforeach; ?>
                <?php endif; ?>
              </tbody>
